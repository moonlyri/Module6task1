...
public class CatalogItemController : ControllerBase
{
    ...
    [HttpPost]
    [ProducesResponseType(typeof(GetBrandResponse<int?>), (int)HttpStatusCode.OK)]
    public async Task<IActionResult> GetBrand(GetBrandRequest request)
    {
        var result = await _catalogItemService.GetBrand(request.CatalogBrandId);
        return Ok(new GetBrandResponse<CatalogItem?>() { Id = result });
    }

    [HttpPost]
    [ProducesResponseType(typeof(GetTypeResponse<int?>), (int)HttpStatusCode.OK)]
    public async Task<IActionResult> GetType(GetTypeRequest request)
    {
        var result = await _catalogItemService.GetType(request.CatalogTypeId);
        return Ok(new GetTypeResponse<CatalogItem?>() { Id = result });
    }
}